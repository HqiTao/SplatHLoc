...
class FeatureGaussianModel(nn.Module):
    def setup_functions(self):
        def build_covariance_from_scaling_rotation(scaling, scaling_modifier, rotation):
            L = build_scaling_rotation(scaling_modifier * scaling, rotation)
            actual_covariance = L @ L.transpose(1, 2)
    ...
    def __init__(self, sh_degree : int, high_dim : int):
        super(FeatureGaussianModel, self).__init__()
        self.active_sh_degree = 0
        self.max_sh_degree = sh_degree  
        self._xyz = torch.empty(0)
        self._features_dc = torch.empty(0)
        ...
        self.feat_encoder = nn.Sequential(
            nn.Conv2d(64, high_dim, kernel_size=3, stride=1, padding=1, bias=False),
        ).cuda().eval()
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': [self._rotation], 'lr': training_args.rotation_lr, "name": "rotation"},
            {'params': [self._loc_feature], 'lr':training_args.loc_feature_lr, "name": "loc_feature"},
            {'params': [p for n, p in self.feat_encoder.named_parameters()], 'lr': training_args.feature_lr / 40.0, "name": "feat_encoder"}
        ]
