...
def readColmapSceneInfo(path, feature_type, images, eval, images_to_read=None):
    ...
    if os.path.exists(os.path.join(path, "sparse/0", "list_test_night.txt")):
        with open(os.path.join(path, "sparse/0", "list_test_night.txt")) as f:
            test_images_night = f.readlines()
            test_images_night = [x.strip() for x in test_images_night]
    ...
    if eval:
        ...
        if "oxford-day-and-night" in path:
            print("gjodfsjgo")
            test_cam_night_infos = []
            for cam_info in test_cam_infos:
                if cam_info.image_name in test_images_night:
                    test_cam_night_infos.append(cam_info)
            test_cam_infos = test_cam_night_infos
